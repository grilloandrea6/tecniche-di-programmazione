...
int main() {
    ...
    for(int j = 0; j < m_giornate; j++) {
        ...
        printf("Giornata %d. Indice della squadra al primo posto in classifica: %d.\n",j+1,massimo(punteggi,n_squadre));
    }
...
// ritorna l'indice della prima occorrenza del valore massimo nel vettore vec di dimensione dim
int massimo(int *vec, int dim) {
    int massimo = 0,index;
    for(int i = 0; i < dim; i++) {
        if(vec[i] > massimo) {
